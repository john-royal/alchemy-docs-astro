...
<Aside type="note">
  This guide builds on the concepts from [State Management](../concepts/state).
  Familiarize yourself with how Alchemy handles state before creating a custom
  state store.
</Aside>
...
<Aside type="note">
  For detailed information on Alchemy's serialization system, see the
  [Serialization and Deserialization](../advanced/serde) guide.
</Aside>
