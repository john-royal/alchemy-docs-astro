...
<Aside type="tip">
  By Convention, the name of this exported `const` should match the name of your
...
<Aside type="tip">
  This also enables type inference since `this.destroy()` returns `never`, so
  the type of the resource can be inferred from the return type of the function.
</Aside>
...
See the [Testing](./testing) documentation for a comprehensive walkthrough on how to test your own resources.
